...
public class FileUtil {
    ...
    //byte数据存储文件
    private final static String AUDIO_TXT_BASEPATH = "/" + rootPath + "/txt/";
    ...
    public static String getTxtFileAbsolutePath(String fileName) {
        ...
        String mAudioTxtPath = "";
        if (isSdcardExit()) {
            if (!fileName.endsWith(".txt")) {
                fileName = fileName + ".txt";
            }
            String fileBasePath = Environment.getExternalStorageDirectory().getAbsolutePath() + AUDIO_TXT_BASEPATH;
            File file = new File(fileBasePath);
            if (!file.exists()) {
                file.mkdirs();
            }
            mAudioTxtPath = fileBasePath + fileName;
        }
        return mAudioTxtPath;
    }
